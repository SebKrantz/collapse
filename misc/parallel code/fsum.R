...
fsum.default <- function(x, g = NULL, w = NULL, TRA = NULL, na.rm = TRUE, use.g.names = TRUE, nthreads = 1L, ...) {
  ...
  if(is.atomic(g)) {
    if(is.nmfactor(g)) return(.Call(Cpp_TRA,x,.Call(C_fsum,x,fnlevels(g),g,w,na.rm,nthreads),g,TtI(TRA)))
    g <- qG(g, na.exclude = FALSE)
    return(.Call(Cpp_TRA,x,.Call(C_fsum,x,attr(g,"N.groups"),g,w,na.rm,nthreads),g,TtI(TRA)))
  }
