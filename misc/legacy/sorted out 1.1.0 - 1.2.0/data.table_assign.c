...
Moved out of ?setkey Details section in 1.12.2 (Mar 2019). Revisit this w.r.t. to recent versions of R.
  The problem (for \emph{data.table}) with the copy by \code{key<-} (other than
  ...
  over-allocated memory wasn't really there after \code{key<-}'s copy. \emph{data.table}s now have an attribute \code{.internal.selfref} to catch and warn about such copies.
