...
\usepackage[landscape,top=0.5cm,bottom=0.6cm,left=0.5cm,right=0.5cm,marginparwidth=1.75cm]{geometry}
...
{
     {\fontsize{22}{30}\selectfont \textcolor{Gray}{Advanced and Fast Data Transformation with \emph{collapse}}}{\Huge\ : : CHEAT SHEET} %\\%\small{by Sebastian Krantz} %
     \vspace{2mm}
}
...
\emph{collapse} provides full control to the user for statistical programming - with several ways to reach the same outcome and rich optimization possibilities. The default is \code{na.rm = TRUE}, and implemented at low cost at the algorithm level.  \\ [0.8em]
