...
%\colorbox{gray}{
\textbf{\textit{collapse}} is a C/C++ based package supporting advanced (grouped, weighted, time series, panel data and recursive) statistical operations in R, with very efficient low-level vectorizations across both groups and columns. \newline

It also offers an efficient class-agnostic approach to data transformation in R: handling matrix and data frame based objects in a uniform, object preserving, way, while ensuring seamless compatibility with \textit{dplyr} / (grouped) \textit{tibble}, \textit{data.table}, \textit{xts}, \textit{sf} and \textit{plm} classes for panel data.  \newline

\textit{collapse} provides full flexibility and control to the user for statistical programming - with multiple ways to achieve the same outcome. The package-wide default is \code{na.rm = TRUE}, and implemented at very low cost at the algorithm level.  \newline

See the documentation at \code{help("collapse-documentation")} or \href{https://sebkrantz.github.io/collapse/reference/index.html}{online}. See also the \href{https://fastverse.github.io/fastverse/}{\textit{fastverse}} package/project for a recommended set of complimentary packages.
%}
...
Same thing, also supporting row-wise operations by reference\\
...
% \vspace{1mm}
\itxt{Optimized functions for grouping, ordering, unique values, and for creating and interacting factors}
\newline

\code{GRP()} - create a grouping object (class 'GRP'): pass to \code{g} arg. %\newline
<<>>=
g <- GRP(iris, ~ Species) # or GRP(iris$Species) or GRP(iris["Species"])
fndistinct(iris[1:4], g)  # Pure computation without grouping overhead
@
...
# Fast Statistical Functions also have a grouped_df method
mtcars |> fgroup_by(cyl, vs, am) |> fmedian() |> head(3)
@
...
\code{f[n]unique()} - fast unique values / rows (by \code{cols})\\
...
mtcars |> ftransformv(disp:drat, fsd, w = wt) |> ss(1:2, 1:5)
mtcars |> fcomputev(disp:drat, fsd, g = cyl, w = wt, TRA = 1,
                    keep = .c(cyl, wt)) |> head(2)
mtcars |> fgroup_by(cyl) |> fsummarise(across(disp:drat, fsd, w = wt))
qTBL(mtcars) |> fgroup_by(cyl) |> fsummarise(across(disp:drat,
     function(x) list(Models = list(lm(disp ~., x))), .apply = FALSE))
add_vars(iris) <- num_vars(iris) |> fsum(TRA = '%') |> add_stub("pc_")
@
<<echo=FALSE, include=FALSE>>=
iris <- iris2
@
...
\quad\qquad\ custom = NULL, keep.col.order = TRUE, ...)}\newline

%\textbf{Example} \\

<<>>=
# Population weighted aggregates: mean (numeric) and mode (categorical)
...
\itxt{Common transformations in (panel data) econometrics} \newline

Scaling, Centering and Averaging\\
\code{fscale(x, g, w, na.rm = TRUE, mean = 0, sd = 1, ...)}\\
\code{fwithin(x, g, w, na.rm = TRUE, mean = 0, theta = 1,)}\\
\code{fbetween(x, g, w, na.rm = TRUE, fill = FALSE, ...)}\newline

Higher-Dimensional Centering/Avg. and Linear Prediction\\
\code{fhdwithin(x, fl, w, na.rm = TRUE,\\
\hspace{11mm}\ \      variable.wise = FALSE, ...)}\\
\code{fhdbetween(x, fl, w, na.rm = TRUE, fill = FALSE,\\
\hspace{13mm} variable.wise = FALSE, ...)}\newline
...
<<>>=
# Grouped scaling
iris |> fgroup_by(Species) |> fscale() |> head(3)
STD(iris, ~ Species, stub = FALSE) |> head(3) # Same thing
# Fitting country-level cubic polynomials for PCGDP, LIFEEX and POP
HDW(wlddev, PCGDP + LIFEEX + POP ~ iso3c * poly(year, 3)) |> head(3)
@

% Such as Formula input:
% STD(mtcars, mpg + carb ~ cyl, w = ~ wt)
%
% Examples:
% lm(mpg ~ hp + B(hp, cyl), mtcars) – FE lm
% STD(mtcars, by = ~ cyl, w = ~wt, stub = FALSE)
% HDW(wlddev, PCGDP ~ qF(country)*poly(year, 3))
%
% Fast grouped  bivariate regression slopes:
% mtcars |> fgroup_by(cyl) |>
%  fmutate(dmx = W(mpg)) |>
%  fsummarise(beta = fsum(dmx, hp) %/=%
%                    fsum(dmx^2))
...
\code{\%!=\%}, \code{copyv}, \code{setv}, \code{alloc}, \code{setop}, \code{\%+=\%}, \code{\%-=\%}, \code{\%*=\%}, \code{\%/=\%}, \code{missing\_cases}, \code{na\_insert}, \code{na\_rm}, \code{na\_omit}, \code{vlengths}, \code{vtypes}, \code{vgcd}, \code{frange}, \code{fnlevels}, \code{fnrow}, \code{fncol}, \code{fdim}, \code{seq\_row}, \code{seq\_col}, \code{cinv}
