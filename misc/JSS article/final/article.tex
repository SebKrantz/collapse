...
\hyphenation{mod-i-fi-ca-tions ma-nip-u-la-tion trans-for-ma-tion per-for-mance de-vel-op-ment
             ...
             sub-set-ting de-com-po-si-tion al-go-rith-mic en-hanced mul-ti-ple coun-tries com-pu-ta-tion-al}
...
\end{Schunk}
%
Weighted quantiles have a sub-column level parallel implementation,\footnote{Use \code{set\_collapse(nthreads = \#)} or the \code{nthreads} arguments to \fct{fnth}/\fct{fmedian}/\fct{fmode} (default 1).} and, as shown above, can harness an (optional) optimization utilizing an overall ordering vector (combining groups and the data column) to avoid repeated sorting of the same elements within groups. % \newline
%
...
These metaprogramming restrictions are vindicated by internally complex data manipulation functions producing greater overheads that can noticeably add up in larger scripts:\footnote{\fct{bmark} is a slim wrapper around \fct{bench::mark}. See the Computational details section.}
\begin{Schunk}
\begin{Sinput}
...
An example using the \code{exports} data follows. \emph{Note} that data need not be sorted for indexing.
\begin{Schunk}
\begin{Sinput}
...
\end{Soutput}
\end{Schunk}
\fct{psmat}, for panel-series to matrix, generates a matrix/array from panel data. Thanks to deep indexation, indexed computations work inside arbitrary data masking environments:
\begin{Schunk}
\begin{Sinput}
...
%
\pkg{collapse}'s \href{https://sebkrantz.github.io/collapse/reference/summary-statistics.html}{summary statistics functions} offer a parsimonious yet powerful toolkit to examine complex datasets. A particular focus has been on providing tools for examining longitudinal (panel) data. Recall the World Development panel dataset (\href{https://sebkrantz.github.io/collapse/reference/wlddev.html}{\code{wlddev}}) from Section~\ref{sec:integration}. The function \fct{varying} can be used to examine which of these variables are time-varying.
%
\begin{Schunk}
\begin{Sinput}
...
\begin{Schunk}
\begin{Sinput}
R> descr(wlda15, by = income + LIFEEX ~ OECD)
\end{Sinput}
\begin{Soutput}
...
As evident from previous sections, \pkg{collapse} provides performance-improved or otherwise enhanced versions of functionality already present in base \proglang{R} (like the \emph{Fast Statistical Functions}, \fct{funique}, \fct{fmatch}, \fct{fsubset}, \fct{ftransform}, etc.) and other packages (esp. \pkg{dplyr} \citep{rdplyr}: \fct{fselect}, \fct{fsummarise}, \fct{fmutate}, \fct{frename}, etc.). The objective of being namespace compatible warrants such a naming convention, but this has a syntactical cost, particularly when \pkg{collapse} is the primary data manipulation package. \newline
...
\begin{Soutput}
    expression    min median mem_alloc n_itr n_gc total_time
1     base_int 46.5ms 46.5ms   166.2MB     1    0     46.5ms
2 collapse_int 10.5ms 10.5ms    38.2MB     1    0     10.5ms
\end{Soutput}
\begin{Sinput}
...
\begin{Soutput}
     expression    min median mem_alloc n_itr n_gc total_time
1     base_char 69.8ms 69.8ms   166.2MB     1    0     69.8ms
2 collapse_char 20.6ms 20.6ms    38.2MB     1    0     20.6ms
\end{Soutput}
\begin{Sinput}
...
\begin{Soutput}
    expression     min  median mem_alloc n_itr n_gc total_time
1     base_int 27.28ms 27.28ms    76.3MB     1    1     27.3ms
2 collapse_int  7.93ms  8.01ms    38.2MB     2    0       16ms
\end{Soutput}
\begin{Sinput}
...
\begin{Soutput}
       expression    min median mem_alloc n_itr n_gc total_time
1       base_char 72.4ms 72.4ms   114.5MB     1    0     72.4ms
2 data.table_char 46.5ms 46.5ms    38.1MB     1    1     46.5ms
3   collapse_char   29ms   29ms    38.1MB     1    0       29ms
\end{Soutput}
\end{Schunk}
%
...
\begin{Soutput}
  expression      min   median mem_alloc n_itr n_gc total_time
1          R   6.07ms   6.09ms    7.86KB     2    0     12.2ms
2   collapse 311.31µs 345.01µs    7.86KB    28    0     10.1ms
\end{Soutput}
\begin{Sinput}
...
\begin{Soutput}
  expression      min   median mem_alloc n_itr n_gc total_time
1          R   6.22ms   6.22ms    7.86KB     2    0     12.4ms
2   collapse 329.27µs 341.08µs    7.86KB    27    0     10.3ms
\end{Soutput}
\begin{Sinput}
...
\begin{Soutput}
  expression    min median mem_alloc n_itr n_gc total_time
1         MS 77.5ms 77.5ms   86.04KB     1    0     77.5ms
2   collapse 19.5ms 19.5ms    7.86KB     1    0     19.5ms
\end{Soutput}
\end{Schunk}
%
...
\begin{Soutput}
  expression    min median mem_alloc n_itr n_gc total_time
1          R 6.04ms 6.29ms    7.85MB     2    0     12.6ms
2   collapse  1.3ms 1.39ms    7.67MB     8    0     11.2ms
\end{Soutput}
\end{Schunk}
%
...
\begin{Soutput}
  expression      min   median mem_alloc n_itr n_gc total_time
1      dplyr 369.09ms 369.09ms   48.56MB     1    6    369.1ms
2 data.table   8.36ms   8.46ms   18.93MB     2    0     16.9ms
3   collapse   4.24ms   4.29ms    9.11MB     3    0     12.9ms
\end{Soutput}
\end{Schunk}
%
...
\begin{Soutput}
       expression    min median mem_alloc n_itr n_gc total_time
1      dplyr_mean  1.28s  1.28s   48.56MB     1   23      1.28s
2 data.table_mean  8.9ms 9.05ms   18.93MB     2    0    18.11ms
3   collapse_mean 4.08ms 4.39ms    9.11MB     3    0    13.02ms
\end{Soutput}
\begin{Soutput}
         expression     min  median mem_alloc n_itr n_gc total_time
1      dplyr_median   4.51s   4.51s    52.8MB     1   90      4.51s
2 data.table_median 23.72ms 23.72ms    18.9MB     1    0    23.72ms
3   collapse_median 11.54ms 11.54ms    11.1MB     1    0    11.54ms
\end{Soutput}
\end{Schunk}
%
...
\begin{Soutput}
  expression      min   median mem_alloc n_itr n_gc total_time
1      dplyr 112.36ms 112.36ms   20.18MB     1    3    112.4ms
2 data.table  63.78ms  63.78ms    5.77MB     1    1     63.8ms
3   collapse   6.46ms   6.54ms     6.8MB     2    0     13.1ms
\end{Soutput}
\end{Schunk}
%
...
\begin{Soutput}
        expression     min  median mem_alloc n_itr n_gc total_time
1      dplyr_joins 172.7ms 172.7ms   558.9MB     1    2    172.7ms
2 data.table_joins 204.5ms 204.5ms   490.4MB     1    1    204.5ms
3   collapse_joins  59.1ms  59.1ms    89.7MB     1    2     59.1ms
\end{Soutput}
\end{Schunk}
%
...
\begin{Soutput}
  expression     min  median mem_alloc n_itr n_gc total_time
1      tidyr  82.7ms  82.7ms     251MB     1    2     82.7ms
2 data.table 143.9ms 143.9ms     209MB     1    2    143.9ms
3   collapse  17.7ms  17.7ms     209MB     1    0     17.7ms
\end{Soutput}
\end{Schunk}
%
...
\begin{Soutput}
     expression      min   median mem_alloc n_itr n_gc total_time
1         tidyr 376.32ms 376.32ms   142.4MB     1    5    376.3ms
2    data.table 237.61ms 237.61ms      21MB     1    4    237.6ms
3 collapse_fsum    7.6ms   7.94ms    39.1MB     2    0     15.9ms
4 collapse_itnl   5.05ms   5.08ms    12.4MB     2    0     10.2ms
\end{Soutput}
\end{Schunk}
%
...
Last but not least, \pkg{collapse} users are encouraged to also familiarize themselves with the \href{https://fastverse.github.io/fastverse/}{\pkg{fastverse}},\footnote{Website: https://fastverse.github.io/fastverse/} a suite of complementary high-performance packages for statistical computing and data manipulation in \proglang{R} which offer more advanced tools in several statistical computing domains. The \pkg{fastverse} metapackage also provides a lightweight framework to jointly load and manage these packages, as well as to build customized and fully separate package verses.
