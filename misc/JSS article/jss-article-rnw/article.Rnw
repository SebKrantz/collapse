...
fmean(mtcars$mpg, g = mtcars$cyl, TRA = "fill")
\end{Code}
...
\end{Code}
By default (\code{verbose = 1}), it prints information about the join operation and number of records joined. Users can request the generation of a \code{.join} column (\code{column = TRUE}), akin to \proglang{STATA}'s \code{\_merge} column, indicating the origin of records in the joined table.
%
<<joins>>=
