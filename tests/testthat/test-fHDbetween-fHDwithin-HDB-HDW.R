...
test_that("fHDbetween with two factors performs like lfe::demeanlist", {
  expect_equal(fHDbetween(x, fl), lfe::demeanlist(x, fl, means = TRUE))
  expect_equal(fHDbetween(xNA, fl), lfe::demeanlist(xNA, fl, means = TRUE, na.rm = TRUE))
  expect_visible(fHDbetween(xNA, fl, fill = TRUE))
  expect_equal(fHDbetween(m, gl), lfe::demeanlist(m, gl, means = TRUE))
  expect_equal(fHDbetween(mNA, gl, na.rm = FALSE), lfe::demeanlist(mNA, gl, means = TRUE))
  expect_equal(fHDbetween(mNA, gl), lfe::demeanlist(mNA, gl, means = TRUE, na.rm = TRUE))
  expect_visible(fHDbetween(mNA, gl, fill = TRUE))
  expect_equal(fHDbetween(mtcars, gl), lfe::demeanlist(mtcars, gl, means = TRUE))
  expect_equal(fHDbetween(mtcNA, gl, na.rm = FALSE), lfe::demeanlist(mtcNA, gl, means = TRUE))
  expect_equal(setRownames(fHDbetween(mtcNA, gl)), lfe::demeanlist(mtcNA, gl, means = TRUE, na.rm = TRUE))
...
test_that("fHDwithin with two factors performs like lfe::demeanlist", {
  expect_equal(fHDwithin(x, fl), lfe::demeanlist(x, fl))
  expect_equal(fHDwithin(xNA, fl), lfe::demeanlist(xNA, fl, na.rm = TRUE))
  expect_visible(fHDwithin(xNA, fl, fill = TRUE))
  expect_equal(fHDwithin(m, gl), lfe::demeanlist(m, gl))
  expect_equal(fHDwithin(mNA, gl, na.rm = FALSE), lfe::demeanlist(mNA, gl))
  expect_equal(fHDwithin(mNA, gl), lfe::demeanlist(mNA, gl, na.rm = TRUE))
  expect_visible(fHDwithin(mNA, gl, fill = TRUE))
  expect_equal(fHDwithin(mtcars, gl), lfe::demeanlist(mtcars, gl))
  expect_equal(fHDwithin(mtcNA, gl, na.rm = FALSE), lfe::demeanlist(mtcNA, gl))
  expect_equal(setRownames(fHDwithin(mtcNA, gl)), lfe::demeanlist(mtcNA, gl, na.rm = TRUE))
...
test_that("fHDbetween with only continuous variables performs like basefitted (defined above)", {
  expect_equal(fHDbetween(x, x2), basefitted(x, x2))
  expect_equal(`attr<-`(fHDbetween(xNA, x2), "na.rm", NULL), basefitted(xNA, x2, na.rm = TRUE))
  expect_visible(fHDbetween(xNA, x2, fill = TRUE))
  expect_equal(fHDbetween(m, m), fHDbetween(m, mtcars))
  expect_equal(fHDbetween(m, m), basefitted(m, m))
  expect_equal(`attr<-`(fHDbetween(mNA, m), "na.rm", NULL), basefitted(mNA, m, na.rm = TRUE))
  expect_equal(fHDbetween(mNA, m, fill = TRUE), fHDbetween(mNA, mtcars, fill = TRUE))
  expect_equal(fHDbetween(mtcars, mtcars), fHDbetween(mtcars, m))
  expect_equal(fHDbetween(mtcars, mtcars), qDF(basefitted(mtcars, mtcars)))
  expect_equal(`attr<-`(fHDbetween(mtcNA, mtcars), "na.rm", NULL), qDF(basefitted(mtcNA, mtcars, na.rm = TRUE)))
  expect_equal(fHDbetween(mtcNA, mtcars, fill = TRUE), fHDbetween(mtcNA, m, fill = TRUE))
  expect_equal(fHDbetween(mtcNA, mtcars, variable.wise = TRUE), fHDbetween(mtcNA, m, variable.wise = TRUE))
})

test_that("fHDwithin with only continuous variables performs like baseresid (defined above)", {
  expect_equal(fHDwithin(x, x2), baseresid(x, x2))
  expect_equal(`attr<-`(fHDwithin(xNA, x2), "na.rm", NULL), baseresid(xNA, x2, na.rm = TRUE))
  expect_visible(fHDwithin(xNA, x2, fill = TRUE))
  expect_equal(fHDwithin(m, m), fHDwithin(m, mtcars))
  expect_equal(fHDwithin(m, m), baseresid(m, m))
  expect_equal(`attr<-`(fHDwithin(mNA, m), "na.rm", NULL), baseresid(mNA, m, na.rm = TRUE))
  expect_equal(fHDwithin(mNA, m, fill = TRUE), fHDwithin(mNA, mtcars, fill = TRUE))
  expect_equal(fHDwithin(mtcars, mtcars), fHDwithin(mtcars, m))
  expect_equal(fHDwithin(mtcars, mtcars), qDF(baseresid(mtcars, mtcars)))
  expect_equal(`attr<-`(fHDwithin(mtcNA, mtcars), "na.rm", NULL), qDF(baseresid(mtcNA, mtcars, na.rm = TRUE)))
  expect_equal(fHDwithin(mtcNA, mtcars, fill = TRUE), fHDwithin(mtcNA, m, fill = TRUE))
  expect_equal(fHDwithin(mtcNA, mtcars, variable.wise = TRUE), fHDwithin(mtcNA, m, variable.wise = TRUE))
})
...
test_that("fHDbetween with multiple variables performs like lm", {
  expect_equal(fHDbetween(iris$Sepal.Length, iris[-1]), `names<-`(fitted(lm(Sepal.Length ~., iris)), NULL))
  expect_equal(fHDbetween(iris[1], iris[-1])[[1]], `names<-`(fitted(lm(Sepal.Length ~., iris)), NULL))
  expect_equal(setRownames(qM(fHDbetween(iris[1:2], iris[-(1:2)]))), fitted(lm(cbind(Sepal.Length, Sepal.Width) ~., iris)))

  expect_equal(`attributes<-`(fHDbetween(data$PCGDP, data[-5]), NULL), `attributes<-`(fitted(lm(PCGDP ~., data)), NULL))
  expect_visible(fHDbetween(data$PCGDP, data[-5], fill = TRUE))
  expect_equal(`attributes<-`(fHDbetween(data[5], data[-5])[[1]], NULL), `attributes<-`(fitted(lm(PCGDP ~., data)), NULL))
  expect_visible(fHDbetween(data[5], data[-5], fill = TRUE))

  expect_equal(setRownames(qM(fHDbetween(data[5:6], data[-(5:6)]))), setRownames(fitted(lm(cbind(PCGDP, LIFEEX)  ~., data))))
  expect_visible(fHDbetween(data[5:6], data[-(5:6)], fill = TRUE))
  expect_visible(fHDbetween(data[5:6], data[-(5:6)], variable.wise = TRUE))

  expect_equal(setRownames(qM(fHDbetween(data[5:7], data[-(5:7)]))), setRownames(fitted(lm(cbind(PCGDP, LIFEEX, ODA)  ~., data))))
  expect_visible(fHDbetween(data[5:7], data[-(5:7)], fill = TRUE))
  expect_visible(fHDbetween(data[5:7], data[-(5:7)], variable.wise = TRUE))

  expect_equal(setRownames(qM(fHDbetween(data[5:6], data$ODA))), setRownames(fitted(lm(cbind(PCGDP, LIFEEX)  ~., data[5:7]))))
  expect_equal(fHDbetween(data[5:6], data[7], fill = TRUE), fHDbetween(data[5:6], data$ODA, fill = TRUE))
  expect_equal(fHDbetween(data[5:6], data[7], variable.wise = TRUE), fHDbetween(data[5:6], data$ODA, variable.wise = TRUE))
})

test_that("fHDwithin with multiple variables performs like lm", {
  expect_equal(fHDwithin(iris$Sepal.Length, iris[-1]), `names<-`(resid(lm(Sepal.Length ~., iris)), NULL))
  expect_equal(fHDwithin(iris[1], iris[-1])[[1]], `names<-`(resid(lm(Sepal.Length ~., iris)), NULL))
  expect_equal(setRownames(qM(fHDwithin(iris[1:2], iris[-(1:2)]))), resid(lm(cbind(Sepal.Length, Sepal.Width) ~., iris)))

  expect_equal(`attributes<-`(fHDwithin(data$PCGDP, data[-5]), NULL), `attributes<-`(resid(lm(PCGDP ~., data)), NULL))
  expect_visible(fHDwithin(data$PCGDP, data[-5], fill = TRUE))
  expect_equal(`attributes<-`(fHDwithin(data[5], data[-5])[[1]], NULL), `attributes<-`(resid(lm(PCGDP ~., data)), NULL))
  expect_visible(fHDwithin(data[5], data[-5], fill = TRUE))

  expect_equal(setRownames(qM(fHDwithin(data[5:6], data[-(5:6)]))), setRownames(resid(lm(cbind(PCGDP, LIFEEX)  ~., data))))
  expect_visible(fHDwithin(data[5:6], data[-(5:6)], fill = TRUE))
  expect_visible(fHDwithin(data[5:6], data[-(5:6)], variable.wise = TRUE))

  expect_equal(setRownames(qM(fHDwithin(data[5:7], data[-(5:7)]))), setRownames(resid(lm(cbind(PCGDP, LIFEEX, ODA)  ~., data))))
  expect_visible(fHDwithin(data[5:7], data[-(5:7)], fill = TRUE))
  expect_visible(fHDwithin(data[5:7], data[-(5:7)], variable.wise = TRUE))

  expect_equal(setRownames(qM(fHDwithin(data[5:6], data$ODA))), setRownames(resid(lm(cbind(PCGDP, LIFEEX)  ~., data[5:7]))))
  expect_equal(fHDwithin(data[5:6], data[7], fill = TRUE), fHDwithin(data[5:6], data$ODA, fill = TRUE))
  expect_equal(fHDwithin(data[5:6], data[7], variable.wise = TRUE), fHDwithin(data[5:6], data$ODA, variable.wise = TRUE))
})
...
test_that("HDW data.frame method (formula input) performs properly", {
  ...
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcars, ~ carb + gear + wt, stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + carb + gear + wt, mtcars))[2:3])
  ...
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcars, ~ factor(cyl) + factor(vs) + factor(am), stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + factor(cyl) + factor(vs) + factor(am), mtcars))[2:3])
  ...
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcars, ~ factor(cyl):factor(vs):factor(am), stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + factor(cyl):factor(vs):factor(am), mtcars))[2:3])
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcars, ~ factor(cyl):factor(vs):factor(am), stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp , W(mtcars, ~ cyl + vs + am, stub = FALSE)))[2:3])
  ...
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcars, ~ factor(cyl) + factor(vs) + factor(am) + carb + gear + wt, stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + factor(cyl) + factor(vs) + factor(am) + carb + gear + wt, mtcars))[2:3])
  ...
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcars, ~ factor(cyl):factor(vs) + factor(am):carb + wt, stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + factor(cyl):factor(vs) + factor(am):carb + wt, mtcars))[2:3])
  ...
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcars, ~ factor(cyl) + factor(vs):poly(gear,2) + factor(am):carb + wt, stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + factor(cyl) + factor(vs):poly(gear,2) + factor(am):carb + wt, mtcars))[2:3])
...
test_that("HDW data.frame method (formula input) with 2-sided formula and missing values performs properly", {
  ...
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcNA, mpg + hp + disp ~ carb + gear + wt, stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + carb + gear + wt, mtcNA))[2:3])
  # continuous interaction
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcNA, mpg + hp + disp ~ carb*gear + wt, stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + carb*gear + wt, mtcNA))[2:3])
  # continuous 3-way interaction
  ...
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcNA, mpg + hp + disp ~ factor(cyl) + factor(vs) + factor(am), stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + factor(cyl) + factor(vs) + factor(am), mtcNA))[2:3])
  # HD fixed effects + factor interaction
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcNA, mpg + hp + disp ~ factor(cyl):factor(vs) + factor(am), stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + factor(cyl):factor(vs) + factor(am), mtcNA))[2:3])
  # 3 way factor interaction
  ...
  # HD fixed effects and continuous variables and full interactions
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcNA, mpg + hp + disp ~ factor(cyl) + factor(vs)*gear + factor(am):carb + wt, stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + factor(cyl) + factor(vs)*gear + factor(am):carb + wt, mtcNA))[2:3], tolerance = 1)
  # HD fixed effects and continuous variables and factor-continuous interactions + factor interactions
  expect_equal(coef(lm(mpg ~ hp + disp, HDW(mtcNA, mpg + hp + disp ~ factor(cyl):factor(vs) + factor(am):carb + wt, stub = FALSE)))[2:3],
               coef(lm(mpg ~ hp + disp + factor(cyl):factor(vs) + factor(am):carb + wt, mtcNA))[2:3])
  # 3-way interaction continuous-factor: error
