...
add_vars(wlddev, c(10L,12L,14L,16L)) <- get_vars(wlddev, 9:12) %>% lapply(log10) %>% add_stub("log10.")
head(wlddev, 2)
rm(wlddev)
```
...
add_vars(wlddev,  get_vars(wlddev, 9:12) %>% lapply(log) %>% add_stub("log."), 
         ...
         pos = c(10L,13L,16L,19L,11L,14L,17L,20L)) %>% head(2)
...
```
By default these functions drop all uncessessary attributes from matrices or lists / data frames in the conversion, but this can be changed using the `keep.attr = TRUE` argument. 
...
Suppose we only want to aggregate the 4 series in this dataset. 
```{r, eval=NCRAN}
...
collap(wlddev, ~ iso3c + decade, w = abs(rnorm(fnrow(wlddev))),
...
For this reason *collapse* inroduces a set of efficient row- and column-wise arithmetric operators for matrix-like objects: `%rr%`, `%r+%`, `%r-%`, `%r*%`, `%r/%`, `%cr%`, `%c+%`, `%c-%`, `%c*%`, `%c/%`. 
...
As mentioned, calling the `TRA()` function does not make much sense if the same task can be performed using the *Fast Statistical Functions* or the arithmetic operators. It is however a very useful function to call for complex transformations involving grouped sweeping operations with precompted quantities. 
...
```{r, eval=NCRAN}
# Center the 4 series in this dataset by country
