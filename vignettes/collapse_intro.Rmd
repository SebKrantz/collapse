...
data <- tfm(data, Time = as.numeric(Time)) %>% # Coercing Time to numeric (from character)
        melt(1:2)                              # Using data.table's melt
head(data)
...
```{r}
# Slightly more efficient than above...
data <- unlist2d(pFEVD, idcols = "variable", id.factor = TRUE, DT = TRUE) %>% 
             tfm(Time = rowidv(., 1)) %>% melt(c("variable", "Time"), variable.name = "Sector") 
head(data)
