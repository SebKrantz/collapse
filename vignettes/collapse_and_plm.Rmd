...
```
Even further customization (i.e. setting means and standard deviations for each group and / or each column) can of course be achieved by calling `TRA` on the result of `fscale` to sweep out an approporiate set of means and standard deviations. 

Scaling without centerin can be done with the option `mean = FALSE`. This will also preserve the mean of the data overall and within each group:
```{r}
