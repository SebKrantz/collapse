...
subtitle: "Facilitating and speeding up (weighted) aggregations, transformations and time-series / panel-data computations in *dplyr*"
...
where `w` is a weight variable (available only to `fmean, fmode, fvar` and `fsd`), and `TRA` and can be used to transform `x` using the computed statistics and one of 8 available transformations (`"replace_fill", "replace", "-", "-+", "/", "%", "+", "*"`). These transformations perform grouped raplacing or sweeping out of the groupwise-statistics comuted by the function, discussed in section 2. `na.rm` efficiently removes missing values and is `TRUE` by default. `use.g.names` generates new row-names from the unique combinations of groups (default: disabled), whereas `keep.group_vars` (default: enabled) will keep the grouping columns as is custom in the native `data %>% group_by(...) %>% summarize(...)` workflow in *dplyr*. Finally, `keep.w` regulates whether the weighting variables used is also aggregated and saved in a column. For `fmean, fvar and fsd` this will compute the sum of the weights within each group, whereas `fmode` will return the maximum weight (corresponding to the mode) in each group. 
...
It is important to not use *dplyr*'s `summarize` together with the fast functions since that would totally eliminate their speed gain. These functions are fast because they are executed only once and carry out the grouped computations in C++, whereas `summarize` will split and then apply the function to each group in the grouped tibble. - It will work, but is slower than using primitive base functions -. 

***

#### Excursus: What is Happening Behind the Scenes?
To drive this point home it is good to shed some light on what is happening behind the scenes of *dplyr* and *collapse*. Fundamentally both packages follow different computing paradigms. 

*dplyr* is an efficient implementation of the Split-Apply-Combine computing paradigm. Data is split into groups, these data-chuncks are then passed to a function carrying out the computation, and finally recombined to produce the aggregated data.frame. The efficiency of that process depends on the efficiency of the grouping, splitting, the function(s) applied and the recombining. This modus operandi is evident in the grouping mechanism of *dplyr*. When a data.frame is passed through *group_by*, a 'groups' attribute is attached: 

```{r}
GGDC10S %>% group_by(Variable,Country) %>% attr("groups")
```

That grouping object is a data.frame giving the unique groups and in the third (last) column vectors containing the indices of the rows belonging to that group. A command like `summarize` uses this information to split the data.frame into groups which are then passed sequentially to the function used and later recombined. 

Now *collapse* is based around one-pass grouped computations at the C++ level, in other words the data is not split and recombined but the entire computation is performed in a single C++ loop running through that data and completing multiple computations simultaneously - one for each group. This modus operandi is also evident in *collapse* grouping objects. The method `GRP.grouped_df` takes a *dplyr* grouping object from a grouped tibble and efficiently converts it to a *collapse* grouping object: 

```{r}
GGDC10S %>% group_by(Variable,Country) %>% GRP %>% str
```

It is a list where the first three elements give the number of groups, the group-id to which each row belongs and a vector of group-sizes. A function like *fsum* uses this information to (for each column) create a result vector of size 'N.groups' and the run through the column using the 'group.id' vector to add the i'th data point to the 'group.id[i]'th element of the result vector. When the loop is finished, the grouped computation is also finished. 

It is clear that *collapse* is prone to be faster than *dplyr* since it's method of computing involves less (and less computationally intensive) steps. This performance gain is realized especially as data become large, since the conversion perfomed by `GRP.grouped_df` also involves a small computational cost. 
...
One can also combine functions in combination with `cbind`, but then the programming becomes a bit verbose. In particular, one needs to use curly braces `{` to prevent first argument injection so that `%>% cbind(FUN1(.), FUN2(.))` does not evaluate as `%>% cbind(., FUN1(.), FUN2(.))`: 
...
A more compact solution to multi-function and multi-type aggregation with *dplyr* is offered by the function *collapg*: 
...
Weighted aggregations are possible with the functions `fmean, fmode, fvar` and `fsd`. The implementation is such that by default (option `keep.w = TRUE`) these functions also aggregate the weights, so that further weighted computations can be performed on the aggregated data. `fmean`, `fsd` and `fvar` compute a grouped sum of the weight column and place it next to the group-identifiers, whereas `fmode` computes the maximum weight (corresponding to the mode) and saves it.

```{r}
# This compute a frequency-weighted group standard-deviation, taking the total EMP / VA as weight
GGDC10S %>% 
  group_by(Variable,Country) %>%
  select_at(6:16) %>% fsd(SUM)

# This compute a weighted group mode, taking the total EMP / VA as weight
GGDC10S %>% 
  group_by(Variable,Country) %>%
  select_at(6:16) %>% fmode(SUM)
```

The weighted variance / standard deviation is corrently only implemented with frequency weights. Reliability weights may be implemented in a further update of *collapse*, if this is a strongly requested feature. 

Weighted aggregations may also be performed with `collapg`, although this presently does not aggregate and save the weights. 
...
Thus to aggregate the data and save the weights one needs to opt for a manual solution: 
...
Below I  provide a set of benchmarks for the standard set of functions commonly used in aggregations. For this purpose I duplicate and row-bind the `GGDC10S` dataset used so far 200 times to yield a dataset of approx. 1 million observations, while keeping the groups unique. 
...
# Selection 
system.time(select_at(data, 6:16))
system.time(get_vars(data, 6:16))
...
system.time(GRP(data)) # It's about 5-10 milliseconds
...
A further highlight of *collapse* is the extremely fast statistical mode function, which can also compute a weighted mode. Fast categorical aggregation has been an issue in R, defining a mode function from base R and applying it to 17000 groups will probably let it run at least a minute. *fmode* reduces this time to half a second. 

Thus in terms of data aggregation *collapse* fast functions are able to speed up *dplyr* to a level that makes it attractive again to R users working on medium-sized or larger data, and everyone programming with *dplyr* (I strongly recommend *collapse* itself for easy and speedy programming as it does not rely on non-standard evaluation and has a lot less R-overhead than *dplyr*). In all this the grouping system of *dplyr* remains the central bottleneck, for example grouping 10 million observations in 1 million groups takes around 10 second with `group_by`, whereas `GRP` takes around 1.5 seconds, and this difference grows exponentially as data get larger. Rewriting `group_by` using `GRP` / *data.table*'s `forderv` and then writing a simple C++ conversion program for the grouping object could be a quick remedy for this issue, but that is at the discretion of Hadeley and the other authors of *dplyr* (If you need that speed program with *collapse* or use *data.table* with GeForce optimized functions).
...
```{r}
# This subtracts the median value from all data points
...
# Computing Percentages of Sectoral data by Variable and Country
GGDC10S %>% 
  group_by(Variable,Country) %>%
    select_at(6:16) %>% fsum("%")
...
Alternatively we could also replace data points with their groupwise mean or standard deviation:

```{r}
# This conducts a simple between transformation (replacing with weighted mean)
GGDC10S %>% 
  group_by(Variable,Country) %>%
...
Of course it is also possible to combine multiple functions as in the aggregation section, or add variables to existing data, as shown below: 

```{r}
# This group-centers data on the group-medians and adds the new variables next to the original ones
...
Certainly There are lots of other examples one could construct using the 8 operations and 13 functions listed above, the examples above just outline the suggested programming basics. 

<!-- ***could add add_vars example again*** -->

### 2.2 More Control using the `TRA` Function

Behind the scenes the fast functions first compute the grouped statistics on all columns of the data, and these statistics are then directly fed into a C++ function that uses them to replace or sweep them out of data points in one of the 8 ways described above. This function can also be called directly by the name of `TRA` (for 'transforming' data by replacing or sweeping out statistics). Foundationally, `TRA` is a generalization of `base::sweep` for column-wise grouped operations^[Row-wise operations are not yet supported by TRA.]. Direct calls to `TRA` enable more control over inputs and outputs. 
...
GGDC10S %>% # This does a proper job
  group_by(Variable,Country) %>% select_at(6:16) %>% fwithin(SUM, add.global.mean = TRUE) 
```

The sequential scaling and centering `... %>% fsd(TRA = "/") %>% fmean(TRA = "-")` shown in an earlier example is also not the best way of doing things. The function `fscale` does this much quicker:
...
### 2.4 Lags, Differences and Growth Rates

It was suggested some time ago that leaving the best wine for the end is not he best strategy when giving a feast. Considering the present marriage of *collapse* and *dplyr* the 3 functions for time-computations introduced in this section combine great flexibility with precision and raw computing power, and feature amongst the highlights of *collapse*. 

The first function, `flag`, computes sequences of lags and leads on time-series and panel-data. `fdiff` computes sequences of lagged-leaded and iterated differences on time-series and panel-data, and `fgrowth` computes lagged-leaded and iterated growth-rates obtained via the exact computatin method or through log-differencing. In addition: None of these functions require the data to be sorted, they can carry out fast computations on completely unordered data as long as a time-variabe is supplied that uniquely identifies the data. 
...
`fdiff` and `fgrowth` can also perform leaded (forward) differences and growth rates, although I have never come to employ these in my personal work. Again it is possible to perform sequential operations: 
...
system.time(mutate_all(data, scale))
