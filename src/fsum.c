...
void fsum_weights_impl_omp(double *pout, double *px, int ng, int *pg, double *pw, int narm, int l, int nth) {
  if(ng == 0) {
    double sum;
    if(narm) {
      int j = 0;
...
double fsum_int_impl_omp(int *px, int narm, int l, int nth) {
  ...
  if(narm) {
    ...
    sum = (long long)px[j];
    #pragma omp parallel for num_threads(nth) reduction(+:sum)
    for(int i = j; i < l; ++i) if(px[i] != NA_INTEGER) sum += (long long)px[i];
  } else {
    sum = 0;
    #pragma omp parallel for num_threads(nth) reduction(+:sum)
...
SEXP fsumC(SEXP x, SEXP Rng, SEXP g, SEXP w, SEXP Rnarm, SEXP Rnth) {
  ...
  if(nwl) {
    ...
    if(nth <= 1) fsum_weights_impl(REAL(out), px, ng, INTEGER(g), pw, narm, l);
    else fsum_weights_impl_omp(REAL(out), px, ng, INTEGER(g), pw, narm, l, nth);
  }
