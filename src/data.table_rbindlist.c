      ...
      for (int j=0; j<ncol; ++j) {
        ...
        if (w!=j && usenames==NA_LOGICAL) {
          ...
          if (!isString(s) || i==0) error("Internal error: usenames==NA but an out-of-order name has been found in an item with no names or the first item. [%d]", i);
           snprintf(buff, sizeof(buff), "Column %d ['%s'] of item %d appears in position %d in item %d. Set use.names=TRUE to match by column name, or use.names=FALSE to ignore column names.%s",
...
//      }
  ...
  for(int j=0; j<ncol; ++j) {
    ...
    if (!foundName) { static char buff[12]; snprintf(buff, sizeof(buff), "V%d", j+1), SET_STRING_ELT(ansNames, idcol+j, mkChar(buff)); foundName=buff; }
