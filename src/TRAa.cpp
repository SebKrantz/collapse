...
// [[Rcpp::export]]
SEXP TRAmCpp(SEXP x, SEXP xAG, IntegerVector g = 0, int ret = 1) {
  int gs = g.size();
  ...
  if(ret <= 2) {
    ...
    case REALSXP:
    {
      ...
      if(gs == 1) {
        ...
        if(AG.size() != col) stop("length(xAG) must match ncol(x)");
        switch(ret) {
        case 1: 
        ...
        if(AG.ncol() != col) stop("ncol(xAG) must match ncol(x)");
    ...
    case INTSXP:
    {
      ...
      if(gs == 1) {
        ...
        NumericMatrix AG = xAG;
        if(AG.ncol() != l) stop("ncol(xAG) must match ncol(x)");
        if(gs != l) stop("g must match nrow(x)");
        switch(ret) {
        case 3: 
          for(int j = col; j--; ) {
