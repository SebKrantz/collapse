...
int dupVecSecond(int *restrict pidx, int *restrict pans_i, SEXP x, const int n, const int ng) {
  ...
  case INTSXP: {
    const int *restrict px = INTEGER(x);
    for (int i = 0; i != n; ++i) {
      id = HASH((unsigned long long)px[i] * (unsigned)pidx[i], K) + pidx[i]; // Need multiplication here instead of bitwise, see your benchmark with 100 mio. obs where second group is just sample.int(1e4, 1e8, T), there bitwise is very slow!!
