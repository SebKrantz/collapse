...
f <- factor(sample(1:100000, 1000000, replace = TRUE), levels = 1:100000)
v <- rnorm(1000000)

head(fgmean(v, f, 100000, collapse = 1))
head(ave(v,f))

system.time(fgmean(v, f, 100000, collapse = 2))
system.time(ave(v,f))



head(demeanlist(as.matrix(v),list(f)))
head(demeanlist(v,list(f)))
head(v - ave(v,f))

system.time(demeanlist(v,list(f))) # A lot faster!!
system.time(v - ave(v,f))
            #,v - ave(v,f))

# About equal speed!! -> Do demeanlist!!!
microbenchmark(demeanlist(v,list(f), threads = 2),fgmean(v, f, 100000, collapse = 2))

# Can still make this function better!!!
mf <- model.frame( ~ factor(cyl)*carb + carb:am, data = mtcars)
...
getfl <- function(mf) {
  ...
  if(any(facts)) { 
    ...
    if(length(modelterms)) {
      ...
      moddat <- .External2(MM, modelterms, # removes NA's?? -> Nope !!
    ...
    fctdat <- NULL
    moddat <- .External2(MM, attr(mf, "terms"), mf) # model.matrix(attr(mf, "terms"), data = mf)
  }
...
subsetfl <- function(fl, cc) {
  lapply(fl, function(f) { # use CsubsetDT or CsubsetVector ?? also check NA in regressors ?? 
    x <- attr(f, "x")
    if(is.null(x)) return(.Call(CSV, f, cc)) else 
      return(`attr<-`(.Call(CSV, f, cc), "x",
...
getPartData <- function(X, fl, Xcols, na.rm, variable.wise) {
  ...
  if(!twosided && Xcolsl) {
    ...
    if(form) { # fl is formula
      ...
      res[[4]] <- Xvars
      if(na.rm) {
        cc <- if(variable.wise) which(!.Call(NADT, X, fvars)) else which(!.Call(NADT, X, c(Xvars,fvars)))
        if(length(cc) != nrow(X)) {
    ...
    fcl <- any(!fc)
    if(na.rm) {
      cc <- if(variable.wise) which(!.Call(NADT, fl, seq_along(fl))) else 
                              which(!.Call(NADT, c(X,fl), seq_len(length(X)+length(fl))))
    ...
    if(na.rm) {
      ...
      if(length(cc) != nrow(X)) {
        if(is.factor(fl)) res[[1]] <- list(.Call(CSV, fl, cc)) else res[[2]] <- cbind(Intercept = 1L, .Call(CSV, fl, cc))
        res[[3]] <- cc
      } else {
...
HDW.default <- function(X, fl, w = NULL, na.rm = TRUE, fill = FALSE, ...) {
  ...
  if(is.list(fl)) {
    ...
    fcl <- any(fc)
    nallfc <- !all(fc)
    if(na.rm) fl <- if(is.data.frame(fl)) .Call(CSDT, fl, cc, seq_along(fl)) else subsetfl(fl, cc) 
    if(fcl && nallfc) { 
      fl <- fl[fc]
...
HDW.data.frame <- function(X, fl, w = NULL, Xcols = is.numeric, na.rm = TRUE, fill = FALSE, variable.wise = FALSE, ...) { 
  ...
  # Best solution ????:
  if(na.rm && !fill) X <- .Call(CSDT, X, cc, Xvars) else if(na.rm && fill && !variable.wise) 
    X <- .Call(CSDT, X, NULL, Xvars) else if(!identical(Xvars,seq_along(X))) X <- X[Xvars]
    Xvars <- seq_along(X)
    ...
    if(!is.null(dat)) {
      if(na.rm && fill) {
        if(variable.wise) {
          ...
          if(length(fl)) { # fastest so far !! (and memory efficient)
            ...
            return(.Call(AS, .Call(AS, X, cc, Xvars, NULL, Y, FALSE),
      ...
      if(na.rm && fill) {
        if(variable.wise) {
          ...
          attr(X, "cases") <- cc
          Y <- lfe::demeanlist(.Call(CSDT, X, cc, Xvars), fl, weights = w[cc], ...)
          return(.Call(AS, .Call(AS, X, cc, Xvars, NULL, Y, FALSE),
                       seq_row(X)[-cc], Xvars, NULL, NA, FALSE))
        }
      } else {
...
HDW.pdata.frame <- function(X, w = NULL, Xcols = is.numeric, na.rm = TRUE, fill = TRUE, 
  ...
  fillcomp <- function(X, fl, w, cc, Xvars, ...) {
    if(length(cc) != length(X[[1]])) {
      Y <- lfe::demeanlist(.Call(CSDT, X, cc, Xvars), lapply(fl, function(x) x[cc]), weights = w[cc], ...)
      return(.Call(AS, .Call(AS, X, cc, Xvars, NULL, Y, FALSE),
                   seq_row(X)[-cc], Xvars, NULL, NA, FALSE))
  ...
  nonfillcomp <- function(X, fl, w, cc, Xvars, ...) {
    if(length(cc) != length(X[[1]])) 
      return(lfe::demeanlist(.Call(CSDT, X, cc, Xvars), lapply(fl, function(x) x[cc]), weights = w[cc], ...)) else 
  ...
  if(na.rm && fill) {
    if(variable.wise) {
      ...
      cc <- which(!.Call(NADT, X, seq_along(X)[-c(gn,gn2)])) # good?? 
    ...
    if(na.rm) {
      ...
      if(length(gn)) {
        if(drop.xt) {
          ...
          ax[["names"]] <- c(nam[gn], if(give.names) paste0("HDW.",nam[-gn2]) else nam[-gn2])
          return(`attributes<-`(c(.Call(CSDT, X, cc, gn), nonfillcomp(X, ax[["index"]], w, cc, seq_along(X)[-gn2], ...)), ax))
        }
      } else {
        if(give.names) paste0("HDW.",nam)
...
# Theory: y = ß1 x1 + ß2 x2 + e
# FWT: M2 y = ß1 M2 x1 + e so residuals: e = M2 y - ß1 M2 x1 and fitted: 
# Now M = I - x(x'x)-1x' = I - P.
# So (I-P2) y = ß1 (I-P2) x1 + e or y - P2 y = ß1 x1 - ß1 P2 x1 + e

# I want y - e = y^ = ß1 x1 + ß2 x2 
# so 
# P2 y = ß1 P2 x1 + ß2 x2
    ...
    if(!is.null(dat)) {
      if(na.rm && fill) {
        if(variable.wise) {
          ...
          if(length(fl)) { # fastest so far !! (and memory efficient)
            dat <- qr(lfe::demeanlist(dat[,-1], fl, weights = w[cc], ...)) # This is necessary!!!
            Y <- lapply(.Call(CSDT, X, cc, Xvars), function(x) x - qr.resid(dat, lfe::demeanlist(x, fl, weights = w[cc], ...))) # good ?? 
            return(.Call(AS, .Call(AS, X, cc, Xvars, NULL, Y, FALSE),
                         seq_row(X)[-cc], Xvars, NULL, NA, FALSE))
          } else {
            dat <- qr(dat)
            Y <- lapply(.Call(CSDT, X, cc, Xvars), function(x) qr.fitted(dat,x))
            return(.Call(AS, .Call(AS, X, cc, Xvars, NULL, Y, FALSE),
                         seq_row(X)[-cc], Xvars, NULL, NA, FALSE))
          }
        }
      ...
      if(na.rm && fill) {
        if(variable.wise) {
          ...
          attr(X, "cases") <- cc
          Y <- lfe::demeanlist(.Call(CSDT, X, cc, Xvars), fl, weights = w[cc], means = TRUE, ...)
          return(.Call(AS, .Call(AS, X, cc, Xvars, NULL, Y, FALSE),
                       seq_row(X)[-cc], Xvars, NULL, NA, FALSE))
        }
      } else {
