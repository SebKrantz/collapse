...
fmode.default <- function(x, g = NULL, w = NULL, TRA = NULL, na.rm = .opt[["na.rm"]], use.g.names = TRUE, ties = "first", nthreads = .opt[["nthreads"]], ...) {
...
fmode.matrix <- function(x, g = NULL, w = NULL, TRA = NULL, na.rm = .opt[["na.rm"]], use.g.names = TRUE, drop = TRUE, ties = "first", nthreads = .opt[["nthreads"]], ...) {
  r <- switch(ties, first = 0L, min = 1L, max = 2L, last = 3L, stop("Unknown ties option: ", ties))
  if(!is.null(g)) g <- GRP(g, return.groups = use.g.names && is.null(TRA), call = FALSE) # sort = FALSE for TRA: not faster here...
...
fmode.data.frame <- function(x, g = NULL, w = NULL, TRA = NULL, na.rm = .opt[["na.rm"]], use.g.names = TRUE, drop = TRUE, ties = "first", nthreads = .opt[["nthreads"]], ...) {
  r <- switch(ties, first = 0L, min = 1L, max = 2L, last = 3L, stop("Unknown ties option: ", ties))
  if(!is.null(g)) g <- GRP(g, return.groups = use.g.names && is.null(TRA), call = FALSE) # sort = FALSE for TRA: not faster here...
...
fmode.grouped_df <- function(x, w = NULL, TRA = NULL, na.rm = .opt[["na.rm"]], use.g.names = FALSE,
                             keep.group_vars = TRUE, keep.w = TRUE, ties = "first", nthreads = .opt[["nthreads"]], ...) {
  r <- switch(ties, first = 0L, min = 1L, max = 2L, last = 3L, stop("Unknown ties option: ", ties))
