...
fcumsum.default <- function(x, g = NULL, o = NULL, na.rm = .ce[["na.rm"]], fill = FALSE, check.o = TRUE, ...) {
...
fcumsum.pseries <- function(x, na.rm = .ce[["na.rm"]], fill = FALSE, shift = "time", ...) {
  if(!missing(...)) unused_arg_action(match.call(), ...)
  index <- uncl2pix(x)
  g <- index[[1L]]
...
fcumsum.matrix <- function(x, g = NULL, o = NULL, na.rm = .ce[["na.rm"]], fill = FALSE, check.o = TRUE, ...) {
  if(!missing(...)) unused_arg_action(match.call(), ...)
  if(length(o) && check.o) o <- ford(o, g)
...
fcumsum.grouped_df <- function(x, o = NULL, na.rm = .ce[["na.rm"]], fill = FALSE, check.o = TRUE, keep.ids = TRUE, ...) {
  if(!missing(...)) unused_arg_action(match.call(), ...)
...
fcumsum.data.frame <- function(x, g = NULL, o = NULL, na.rm = .ce[["na.rm"]], fill = FALSE, check.o = TRUE, ...) {
  if(!missing(...)) unused_arg_action(match.call(), ...)
  if(length(o) && check.o) o <- ford(o, g)
...
fcumsum.pdata.frame <- function(x, na.rm = .ce[["na.rm"]], fill = FALSE, shift = "time", ...) {
  if(!missing(...)) unused_arg_action(match.call(), ...)
  index <- uncl2pix(x)
  g <- index[[1L]]
  o <- switch(shift, time = ford(index[[2L]], g), row = NULL, stop("'shift' must be either 'time' or 'row'"))
