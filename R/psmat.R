...
psmat.data.frame <- function(x, by, t = NULL, cols = NULL, transpose = FALSE, simplify = FALSE) {
...
psmat.pseries <- function(x, transpose = FALSE) {
  ...
  psmatCpp(x, index[[1]], index[[2]], transpose) 
}
psmat.pdata.frame <- function(x, transpose = FALSE, simplify = FALSE) {
  index <- attr(x, "index")
  if(length(index) > 2L) index <- c(interaction(index[-length(index)], drop = TRUE), index[length(index)])
  res <- lapply(x, psmatCpp, index[[1]], index[[2]], transpose) 
  if(simplify) {
   if(length(res) == 1L) return(res[[1L]]) else return(simplify2array(res)) 
  } else return(res)
}
