...
na_rm2 <- function(x) if(anyNA(x)) x[!is.na(x)] else x
...
radixfact <- function(x, sort, ord, fact, naincl, keep, retgrp = FALSE) {
  ...
  if(fact) {
    ...
    if(naincl) {
      ...
      attr(f, "levels") <- if(attr(o, "sorted")) unattrib(tochar(na_rm2(.Call(C_subsetVector, x, st)))) else
            unattrib(tochar(na_rm2(.Call(C_subsetVector, x, o[st]))))
    }
    ...
    if(retgrp) {
      if(naincl) {
         ...
         attr(f, "groups") <- if(attr(o, "sorted")) na_rm2(.Call(C_subsetVector, x, st)) else na_rm2(.Call(C_subsetVector, x, o[st]))
      }
    }
