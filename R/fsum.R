...
fsum.default <- function(x, g = NULL, w = NULL, TRA = NULL, na.rm = .opt[["na.rm"]], use.g.names = TRUE, fill = FALSE, nthreads = .opt[["nthreads"]], ...) {
...
fsum.matrix <- function(x, g = NULL, w = NULL, TRA = NULL, na.rm = .opt[["na.rm"]], use.g.names = TRUE, drop = TRUE, fill = FALSE, nthreads = .opt[["nthreads"]], ...) {
  if(is.null(TRA)) {
    if(!missing(...)) unused_arg_action(match.call(), ...)
...
fsum.data.frame <- function(x, g = NULL, w = NULL, TRA = NULL, na.rm = .opt[["na.rm"]], use.g.names = TRUE, drop = TRUE, fill = FALSE, nthreads = .opt[["nthreads"]], ...) {
  if(is.null(TRA)) {
    if(!missing(...)) unused_arg_action(match.call(), ...)
...
fsum.grouped_df <- function(x, w = NULL, TRA = NULL, na.rm = .opt[["na.rm"]], use.g.names = FALSE,
                             keep.group_vars = TRUE, keep.w = TRUE, fill = FALSE, nthreads = .opt[["nthreads"]], ...) {
  ...
  if(!is.null(wsym)) {
    ...
    if(length(wn <- which(nam %in% all.vars(wsym)))) {
      ...
      gn <- c(gn, wn)
      if(keep.w) {
        if(nTRAl) sumw <- `names<-`(list(fsumC(w,g[[1L]],g[[2L]],NULL,na.rm,fill)), paste0("sum.", if(length(wsym) == 1L) wsym else deparse(wsym))) else if(keep.group_vars)
