...
* `missing_cases()` and `na_omit()` have gained an argument `all = FALSE`. Setting `all = TRUE` toggles detection/removal of cases that are all-missing. For data frames this is done efficiently in C. For matrices this is currently still implemented using `rowSums(is.na(X)) == ncol(X)`, so the performance is less than optimal. 
