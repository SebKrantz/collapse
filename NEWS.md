...
* `join()` now provides enhanced verbosity, indicating the average order of the join between the two tables, e.g.
  ...
  * In *collap*, with multiple functions passed to `FUN` or `catFUN` and `return = "long"`, the `"Function"` column is now generated as a factor variables instead of character (which is more efficient). 
