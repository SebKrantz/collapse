...
static void cleanup() {
  ...
  if (gs_thread!=NULL) for (int i=0; i<1; i++) free(gs_thread[i]);
...
void radix_r(const int from, const int to, const int radix) {
  ...
  if (my_n==1) {  // minor TODO: batch up the 1's instead in caller (and that's only needed when retgrp anyway)
    ...
    // if (1==1) Rprintf("counting clause: radix=%d, my_n=%d\n", radix, my_n);
    ...
    uint8_t *restrict my_ugrp = UGRP + 1*256;  // uninitialized is fine; will use the first ngrp items. Only used if sortType==0
    ...
    TEND(10)
      ...
      if (!skip) {
        ...
        int *restrict my_TMP = TMP + 1*UINT16_MAX; // Allocated up front to save malloc calls which i) block internally and ii) could fail
        if (radix==0 && nalast!=-1) {
  ...
  int batchSize = MIN(UINT16_MAX, 1+my_n/1);  // (my_n-1)/nBatch + 1;   //UINT16_MAX == 65535
