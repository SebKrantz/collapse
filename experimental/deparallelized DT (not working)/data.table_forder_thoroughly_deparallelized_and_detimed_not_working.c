...
// when sortGroups is TRUE, ascArg contains +1/-1 for ascending/descending of each by column; when FALSE ascArg is ignored
{
  ...
  if (nrow==0) {
    ...
    if (retgrp) {
      setAttrib(ans, sym_starts, allocVector(INTSXP, 0)); // install("starts")
      setAttrib(ans, sym_maxgrpn, ScalarInteger(0)); // install("maxgrpn")
    }
    UNPROTECT(n_protect);
    return ans;
  }
  ...
  for (int col=0; col<ncol; col++) {
    ...
    case REALSXP :
      if (INHERITS(x, char_integer64)) { // PRINTNAME(install("integer64"))
        range_i64((int64_t *)REAL(x), nrow, &min, &max, &na_count);
      } else {
        if (verbose && INHERITS(x, char_Date) && INTEGER(isReallyReal(x))[0]==0) { // PRINTNAME(install("Date"))
    ...
    if (na_count==nrow || (min>0 && min==max && na_count==0 && infnan_count==0)) {
      ...
      if (TYPEOF(x)==STRSXP) free_ustr(); // remove this ??
      continue;
    }
    ...
    case STRSXP : {
      ...
      free_ustr();  // comment out ?? // ustr could be left allocated and reused, but free now in case large and we're tight on ram
      break;
    default:
  ...
  TMP =  (int *)malloc(UINT16_MAX*sizeof(int)); // used by counting sort (my_n<=65536) in radix_r()
  UGRP = (uint8_t *)malloc(256);                // TODO: align TMP and UGRP to cache lines (and do the same for stack allocations too)
  // if (!TMP || !UGRP /*|| TMP%64 || UGRP%64*/) STOP("Failed to allocate TMP or UGRP or they weren't cache line aligned: nth=%d", nth);
  if (nradix) radix_r(0, nrow-1, 0);  // top level recursive call: (from, to, radix)
  ...
  if (retgrp) {
    SEXP tt;
    int final_gs_n = gs_n;   // TODO: find a neater way to do this
    int *final_gs  = gs;
    setAttrib(ans, sym_starts, tt = allocVector(INTSXP, final_gs_n)); //  install("starts")
    int *ss = INTEGER(tt);
    int maxgrpn = 0;
    ...
    setAttrib(ans, sym_maxgrpn, ScalarInteger(maxgrpn)); // install("maxgrpn")
  }
...
void radix_r(const int from, const int to, const int radix) {
  ...
  if (my_n==1) return;
    ...
    uint8_t *restrict my_ugrp = UGRP + 256;  // uninitialized is fine; will use the first ngrp items. Only used if sortType==0
    ...
    if (!skip) {
      ...
      int *restrict my_TMP = TMP + UINT16_MAX; // Allocated up front to save malloc calls which i) block internally and ii) could fail
      if (radix==0 && nalast!=-1) {
  ...
  int batchSize = MIN(UINT16_MAX, 1+my_n);  // (my_n-1)/nBatch + 1;   //UINT16_MAX == 65535
