      ...
      if(narm) {
        ...
        if(B) {
          if(option) std::fill(out.begin(), out.end(), (double)sum); // fastes ??
            ...
            for(int i = 0; i != l; ++i) {
              if(std::isnan(x[i]) || std::isnan(wg[i])) out[i] = NA_REAL;
          ...
          for(int i = 0; i != l; ++i) {
            if(std::isnan(x[i]) || std::isnan(wg[i])) out[i] = NA_REAL;
            else out[i] = x[i] - sum; // double conversion ??
          }
          // out = x - sum; // conversion to double not necessary !! -> faster !!!
        }
      } else {
        long double sum = 0, sumw = 0;
        for(int i = 0; i != l; ++i) {
          if(std::isnan(x[i]) || std::isnan(wg[i])) { // good, check both ?? -> yes!!
            sum = NA_REAL; //  x[i]+wg[i];
            break;
          } else {
            sum += x[i]*wg[i];
            sumw += wg[i];
      ...
      if(narm) {
        ...
        if(B) {
          ...
          if(option) {
            ...
            for(int i = 0; i != l; ++i) {
              if(std::isnan(x[i]) || std::isnan(wg[i])) out[i] = NA_REAL; // x[i];
              else out[i] = sum[g[i]-1];
            }
          }
        } else {
        ...
        for(int i = 0; i != l; ++i) {
          if(std::isnan(x[i]) || std::isnan(wg[i])) {
            if(!std::isnan(sum[g[i]-1])) {
              sum[g[i]-1] = sumw[g[i]-1] = NA_REAL; // x[i]+wg[i];
              ++ngs;
              if(ngs == ng) break;
            }
          } else {
        ...
        for(int j = col; j--; ) { // Instead Am(j,_) you can use Am.row(j).
          ...
          if(B) {
            if(option) std::fill(outj.begin(), outj.end(), (double)sumj);
              ...
              for(int i = 0; i != l; ++i) {
                if(std::isnan(column[i]) || std::isnan(wg[i])) outj[i] = NA_REAL; // column[i];
                else outj[i] = sumj;
              }
            }
          } else {
            for(int i = 0; i != l; ++i) {
              if(std::isnan(column[i]) || std::isnan(wg[i])) outj[i] = NA_REAL; // column[i];
              else outj[i] = column[i] - sumj;
            }
            // outj = column - sumj; -> a lot faster !!
          }
        }
      } else {
        for(int j = col; j--; ) {
      ...
      if(narm) {
        for(int j = col; j--; ) {
          ...
          if(B) {
            ...
            if(!option) {
              for(int i = ng; i--; ) sumj[i] /= sumwj[i];
              for(int i = 0; i != l; ++i) {
                if(std::isnan(column[i]) || std::isnan(wg[i])) outj[i] = NA_REAL;
                else outj[i] = column[i] - sumj[g[i]-1];
              }
              // for(int i = 0; i != l; ++i) outj[i] = column[i] - sumj[g[i]-1];
            } else {
              double osum = 0, osumw = 0;
              ...
              for(int i = 0; i != l; ++i) {
                if(std::isnan(column[i]) || std::isnan(wg[i])) outj[i] = NA_REAL;
                else outj[i] = column[i] - sumj[g[i]-1] + osum;
              }
              // for(int i = 0; i != l; ++i) outj[i] = column[i] - sumj[g[i]-1] + osum;
            }
          }
        ...
        for(int j = col; j--; ) {
          ...
          for(int i = 0; i != l; ++i) {
            if(std::isnan(column[i]) || std::isnan(wg[i])) {
              if(!std::isnan(sumj[g[i]-1])) {
                sumj[g[i]-1] = sumwj[g[i]-1] = NA_REAL; // column[i]+wg[i];
                ++ngs;
                if(ngs == ng) break;
              }
            } else {
      ...
      if(narm) {
        for(int j = l; j--; ) {
          ...
          if(B) {
            ...
            for(int i = 0; i != row; ++i) {
              if(std::isnan(column[i]) || std::isnan(wg[i])) outj[i] = NA_REAL;
              else outj[i] = column[i] - sumi;
            }
            out[j] = outj;
            // out[j] = column - sumi; -> a lot faster !!
          }
